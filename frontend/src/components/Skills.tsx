...
interface Skill {
  ...
  category: string; // Category of the skill (e.g., Core, Styling, Tools, Quality)
  icon?: string; // Optional icon representing the skill
}

/**
 * Skill a component of the application.
 * This component displays a categorized list of technical and soft skills,
 * along with decorative statistics and animations.
 *
...
export function Skills() {
  ...
  const skills: Skill[] = [
    // Core Frontend
    {name: 'Angular', category: 'Core', icon: '🅰️'},
    ...
    {name: 'Sass / CSS', category: 'Styling', icon: '🎨'},
    {name: 'Motion', category: 'Styling', icon: '✨'},
    {name: 'CSS Modules', category: 'Styling', icon: '📦'},

    // State Management & Tools
    {name: 'Git / GitHub', category: 'Tools', icon: '🔱'},
    {name: 'Provider / Redux', category: 'Tools', icon: '🔄'},
    {name: 'Azure Pipelines', category: 'Tools', icon: '🔷'},
    {name: 'Nx Microfrontends', category: 'Tools', icon: '⚡'},

    // Testing & Quality
    {name: 'Jest', category: 'Quality', icon: '🃏'},
    {name: 'Flutter Test', category: 'Quality', icon: '🧪'},
    {name: 'Playwright', category: 'Quality', icon: '🎭'},
    {name: 'Lighthouse', category: 'Quality', icon: '🚀'},
  ];
  ...
  return (<section id="skills" className="min-h-screen flex items-center justify-center px-4 md:px-8 py-20">
    <div className="max-w-7xl mx-auto w-full">
      ...
      >
        <h2 className="gradient-text mb-4">{t('skills.title')}</h2>
            ...
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                ...
                <p className="text-[#718096] text-sm mt-1">
                  {categorySkills.length} {t('skills.technologies')}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              >
                <div
                  ...
                  {/* Hover effect */}
                  <div
                    className={`absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${category.bgColor}`}
                  ></div>
      ...
      >
        <div className="text-center mb-10">
          <h3 className="text-[#2D3748] mb-2">{t('skills.beyond')}</h3>
          <p className="text-[#718096]">{t('skills.beyond.subtitle')}</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {[
            {icon: '👥', label: t('skills.teamwork'), description: t('skills.teamwork.desc')},
            {icon: '👑', label: t('skills.leadership'), description: t('skills.leadership.desc')},
            {icon: '🎯', label: 'Orientado a resultados', description: 'Enfoque en objetivos'},
            // {icon: '💬', label: t('skills.communication'), description: t('skills.communication.desc')},
            {icon: '📘', label: t('skills.learning'), description: t('skills.learning.desc')},
      ...
      >
        {[
          ...
          // {number: '16', label: t('skills.stats.tech')},
          {

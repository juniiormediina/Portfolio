...
import { Code2, Palette, Wrench, CheckCircle } from 'lucide-react';
              ...
              >
                ...
                <div className="flex items-center gap-4 mb-8">
                  <div className={`neomorphic-flat p-4 rounded-2xl ${category.bgColor}`}>
                    <IconComponent className={`w-8 h-8 bg-gradient-to-r ${category.color} bg-clip-text text-transparent`} style={{ stroke: 'url(#gradient)' }} />
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {categorySkills.map((skill, skillIndex) => (
                    ...
                    >
                      <div className="neomorphic p-6 rounded-2xl h-full flex flex-col items-center justify-center gap-3 hover:shadow-2xl transition-all duration-300 cursor-pointer">
                        {/* Efecto de brillo en hover */}
                        <div className={`absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${category.bgColor}`}></div>
                        ...
                        <p className="text-[#2D3748] text-center relative z-10 group-hover:text-[#667EEA] transition-colors duration-300">
                          {skill.name}
                        </p>

                        {/* Indicador decorativo */}
                        <div className="absolute bottom-2 left-1/2 -translate-x-1/2 w-8 h-1 bg-gradient-to-r from-transparent via-[#667EEA]/0 to-transparent group-hover:via-[#667EEA] transition-all duration-300 rounded-full"></div>
                      </div>
                    </motion.div>
                  ))}
                </div>
